/**
 * Обробка заявки з лендінгу → повідомлення в Telegram.
 ...
 *
 * Токен у код не вписувати: репозиторій публічний.
 */
...
/**
 * Діагностика. GET /api/lead — чи задеплоєний обробник і чи заданий BOT_TOKEN.
...
export async function health(request, env) {
  const out = { ok: true, fn: "lead", token_set: Boolean(env.BOT_TOKEN), chat_id: chatOf(env) };
...
/** Спільний роутер для /api/lead. Повертає null, якщо метод не підтримується. */
export function handleLead(request, env) {
